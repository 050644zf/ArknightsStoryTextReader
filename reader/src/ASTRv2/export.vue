        ...
        <n-space vertical class="exportpage">
            ...
            <n-space item-style="display: flex;" align="center" justify="end">
                <n-button @click="removeAll" type="error">
                    <n-icon size="24"><ClearIcon/></n-icon>&nbsp;
                    清空 / Clear
                </n-button>
...
import {MenuOpenFilled, ClearAllOutlined, FileDownloadOutlined} from '@vicons/material';
    ...
    methods:{
        ...
        async exportStories(){
            for(const story of this.exportList){
                    ...
                    this.percentage = Math.ceil(this.exportList.indexOf(story) / this.exportList.length * 100);
                }
        },
        async exportStory(story){
            this.logs += 'Exporting Story: '+story.storyCode + ' ' + story.avgTag + ' ' + story.storyName + '\n';
