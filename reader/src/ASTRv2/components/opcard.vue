<template>
  <n-modal class="opcard font" :show="show" @mask-click="close">
      ...
      <template #header>
        <n-flex align="center">
          ...
          <n-flex vertical>
            ...
            <n-flex>
              <n-tag v-if="cdata.displayNumber"
                >{{ cdata.displayNumber }}
              </n-tag>
            </n-flex>
          </n-flex>
      ...
      <n-tabs type="line" animated>
        <n-tab-pane name="archive" :tab="i18n.archive[currentLang]">
          <n-scrollbar style="max-height: 70vh">
            <div v-for="(aline, aidx) in cdata.storyTextAudio" :key="aidx">
              ...
              <n-text
                v-for="(story, sidx) in aline.stories"
                :key="sidx"
        ...
        <n-tab-pane name="equip" :disabled="cdata.equips.length < 1">
          ...
          <n-collapse accordion :default-expanded-names="[0]">
            <n-collapse-item
              v-for="(equip, eidx) in cdata.equips"
              :key="eidx"
              :name="eidx"
            >
              <template #header>
              ...
              <template #header-extra>
                <n-tag
                  v-if="equip.type == 'ADVANCED'"
                  :color="tagColor(equip.equipShiningColor)"
                >
              ...
              <n-scrollbar style="max-height: 50vh">
                <n-flex vertical align="center">
                  <n-image
                    v-if="equip.type == 'ADVANCED'"
                    :src="getEquipUrl(equip.uniEquipId)"
                    height="200"
                  />
        ...
        <n-tab-pane name="record" :disabled="cdata.handbookAvgList.length < 1">
          ...
          <div v-for="(recs, ridx) in cdata.handbookAvgList" :key="ridx">
            ...
            <n-list hoverable clickable>
              <n-list-item
                v-for="(rec, reidx) in recs.avgList"
                :key="reidx"
                @click="warpToStory(rec.storyTxt)"
              >
                <template #prefix>
...
export default {
  ...
  methods: {
    close() {
      ...
      this.$router.push({
        name: "menu",
        params: { server: server, selected: "or" },
      });
    },
    warpToStory(storyTxt) {
      this.$router.push({
        name: "content",
        params: { server: this.$route.params.server },
        query: { f: storyTxt },
      });
    },
    tagColor(color) {
