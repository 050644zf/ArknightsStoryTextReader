  ...
  <n-layout-content class="act25side">
    <n-space vertical class="isrecords">
      ...
      <n-collapse default-expanded-names="1" accordion>
        <n-collapse-item
          v-for="(report, idx) in menu_options"
          :key="report.key"
          :name="idx"
          :class="{ log: true, expanded: menu_options[idx].expanded }"
        >
          <template #header>
            <n-space>
              <n-icon><ArchiveIcon /></n-icon>
              <n-text style="font-weight: bold !important">{{
                report.label
              }}</n-text>
            </n-space>
          </template>
          <n-space justify="center">
            <n-text
              v-html="parseContent(report.item)"
              class="log_text"
            ></n-text>
          </n-space>
        </n-collapse-item>
...
export default {
  ...
  methods: {
    async loadLogData() {
      try {
        var mdata = {};
          mdata = await source.getData(this.server, "/gamedata/excel/story_review_meta_table.json").then((res) => res.json());
        this.mdata = mdata;
        this.log_entries =
