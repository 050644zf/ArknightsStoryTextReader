    ...
    <n-layout-content>
        <n-space vertical class="isrecords">
            <n-breadcrumb class="breadcrumb">
                ...
                <n-breadcrumb-item>月度小队记录 | Monthly Team Records</n-breadcrumb-item>
            </n-breadcrumb>
            <n-collapse>
                <n-collapse-item v-for="(topic, tpidx) in topics" :key="tpidx" >
                    ...
                    <n-space vertical>
                        <n-card v-for="(team, tid, tidx) in idata[topic.id]" :key="tidx" :header-style="{'border-top':'5px solid #'+team.teamColor}">
                            <template #header>
                                <n-space justify="space-between" item-style="display:flex" align="center">
                                    <n-h2 style="margin:0px">{{team.teamYear}}-{{team.teamMonth}} {{team.teamName}}</n-h2>
                                    <n-space>
                                        <img v-for="(charId, cidx) in team.teamChars" :key="cidx" :src="getCharAvatar(charId)" style="width:48px"/>
                                    </n-space>
                                </n-space>
                            </template>
...
export default {
    ...
    methods:{
        async loadIsData(){
            try{
                let idata = await fetch('https://raw.githubusercontent.com/Kengxxiao/ArknightsGameData/master/'+this.server+'/gamedata/excel/roguelike_topic_table.json').then(res => res.json());
