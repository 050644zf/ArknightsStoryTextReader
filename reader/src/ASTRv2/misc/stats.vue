    ...
    <n-layout-content>
        <n-space vertical class="stats" v-if="$route.params.server=='zh_CN'">
            <n-breadcrumb class="breadcrumb">
                ...
                <n-breadcrumb-item>明日方舟特别回顾 | Arknight Journey Review</n-breadcrumb-item>
            </n-breadcrumb>
            ...
            <n-steps>
                <n-step title="登录 / Login">
                    <n-space vertical>
                        ...
                        <n-button @click="openNewTab('https://ak.hypergryph.com/activity/journey-review')">
                            <template #icon>
                ...
                <n-step title="分析 / Analysis" :status="(this.uploaded.stats && this.uploaded.story)?'process':'wait'">
                    <n-space vertical>
                        <n-text>
                            点击按钮开始分析。<br/>
                            Click the button to start analysis.
                        </n-text>
                            <n-button type="info" @click="analysis()" :disabled="!(this.uploaded.stats && this.uploaded.story)">
                                <template #icon>
            ...
            <n-space v-if="analysis_enabled" vertical>
                <n-h2 prefix="bar">统计时间 / Statistic Period: 2019/04/30 16:00 - 2022/04/01 00:00 (UTC+8)</n-h2>
                <n-card>
                    <template #header>
                    ...
                    <n-space justify="space-between">
                        ...
                        <n-statistic>
                            <template #label>解锁支线数 <br/> # of Sidestory Unlocked</template>
                            <n-number-animation from="0" :to="story.result_act_engage.act_ss_engage_cnt"/>
                            <!-- {{story.result_act_engage.act_ss_engage_cnt}} -->
                        </n-statistic>
                        <n-statistic>
                            <template #label>解锁故事集数 <br/> # of Storyset Unlocked</template>
                            <n-number-animation from="0" :to="story.result_act_engage.act_mini_engage_cnt"/>
                        </n-statistic>
                        <n-statistic>
                            <template #label>经手简历数 <br/> # of Resumes Handled</template>
                            <n-number-animation from="0" :to="stats.result_char_get.char_get_cnt_histroy"/>
                        </n-statistic>
                        <n-statistic>
                ...
                <n-card>
                    ...
                    <n-space justify="space-between">
                        <n-statistic>
                            <template #label>作战完成数 <br/># of Battles Finished </template>
                            <n-number-animation from="0" :to="stats.result_stage_clear_cnt.battle_finish_cnt_total" show-separator/>
                        </n-statistic>
                        <n-statistic>
                            <template #label>作战理智消耗 <br/>Sanity Consumed in Combat</template>
                            <n-number-animation from="0" :to="stats.result_stage_clear_cnt.ap_battle_use_total" show-separator/>
                        </n-statistic>
                        <n-statistic>
                            <template #label>单日作战最高次数 <br/>Highest # of Battles Finished in one day</template>
                            <n-number-animation from="0" :to="stats.result_stage_clear_cnt.battle_one_day_most_cnt"/>
                            <template #suffix>
                                Times
                                <n-text depth="3">
                                    <br/>
                ...
                <n-card>
                    ...
                    <n-space>
                        ...
                        <n-statistic>
                            <template #label>来自好友的线索数 <br/># of Clues from Friends</template>
                            <n-number-animation from="0" :to="stats.result_clue.clue_in_cnt"/>
                        </n-statistic>
                        <n-statistic>
                            <template #label>送给好友的线索数 <br/># of Clues Sent</template>
                            <n-number-animation from="0" :to="stats.result_clue.clue_out_cnt"/>
                        </n-statistic>
                        <n-statistic>
                            <template #label>给予最多线索的好友 <br/>Friend who give you most clues</template>
                            {{stats.result_clue.friend_clue_in_most_nickname}}
                        </n-statistic>
                        <n-statistic>
                            <template #label>收到最多线索的好友 <br/>Friend who received most clues from you</template>
                ...
                <n-card>
                    ...
                    <n-modal v-model:show="showSecretaryHistory">
                        <n-card style="width: 800px">
                            ...
                            <n-data-table :columns="SecretaryHistoryCols" :data="stats.result_char_secretary_favor.secretary_char_used_list" size="small" class="datatable" virtual-scroll max-height="500"></n-data-table>
                        </n-card>
                    </n-modal>
                    <template #action>
