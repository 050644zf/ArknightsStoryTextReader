        ...
        <n-flex vertical justify="center" align="center">
            <n-flex align="center">
                ...
                <n-radio-group v-model:value="filters.profession">
                    ...
                    <n-radio-button v-for="(prof, pidx) in professions" :key="prof" :value="prof" >
                    
                        <n-image 
                        :src="'https://raw.githubusercontent.com/Aceship/Arknight-Images/main/classes/class_' + professions_aceship[pidx].toLowerCase() + '.png'"
                        width="32"
                        preview-disabled
                        />
                    </n-radio-button>
            ...
            <TransitionGroup name="list">
                <div v-for="(cdata, cidx) in getCharList" :key="cdata.charID">
                    <n-card class="optile" content-style="padding: 0px;" style="display: flex; justify: center; align-items: center;" hoverable  @click="showCard(cdata.charID)" >
                        ...
                        <n-ellipsis class="opcard-name" :tooltip="{'placement':'bottom'}">{{cdata.name}}</n-ellipsis>
                    </n-card>
...
export default{
    ...
    computed:{
        getCharList(){
            ...
            for(var char in this.charinfo){
                ...
                if(this.filters.profession != 'ALL' && this.filters.profession != cdata.profession) continue;
                if(this.filters.rarity && 'TIER_'+ this.filters.rarity != cdata.rarity) continue;
    ...
    methods:{
        async loadCharInfo(){
            ...
            try{
                let cdata = await fetch(gamejson+this.server+'/charinfo.json').then(res => {return res.json();});
                this.charinfo = cdata;
                this.loaded = true;
            }
            catch(e){
